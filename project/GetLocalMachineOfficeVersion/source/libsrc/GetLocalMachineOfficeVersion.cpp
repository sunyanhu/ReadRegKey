#include "GetLocalMachineOfficeVersion.h"
#include <io.h>
#include "osal/Osal.h"
...
#ifdef _DEBUG
#pragma comment(lib, "win_systemd.lib")
#pragma comment(lib, "osald.lib")
#else
#pragma comment(lib, "win_system.lib")
...
CGetLocalMachineOfficeVersion::CGetLocalMachineOfficeVersion()
{
}
...
bool CGetLocalMachineOfficeVersion::SearchRegedit(CRegistryKey regKey, REGSAM regSam)
{
	bool ret = true;
...
bool CGetLocalMachineOfficeVersion::GetSoftwareInfoForPlatform(bool is64Platform)
{
	bool ret = true;
	try
	{
...
bool CGetLocalMachineOfficeVersion::SetOfficeVersion(map<U_OFFICE_TYPE, string> &officeVersion)
{
	bool ret = true;
	...
	for (iter = mSoftwareInfo.begin(); iter != mSoftwareInfo.end(); iter++)
	{
		if (iter->displayName.find("Microsoft Office") != -1)
		{
			...
			case 11:
			{
				...
				string wordexe = exe + "WINWORD.exe";
				...
				string powerpntexe = exe + "POWERPNT.exe";
		...
		if (iter->displayName.find(_T("WPS Office")) != -1)
		{
			...
			if (version.compare(0, 3, "6.4") == 0)
			{
				...
				string powerpntexe = exe + "wpp.exe";
				if (_access(powerpntexe.c_str(), 0) != -1)
				{
...
bool CGetLocalMachineOfficeVersion::GetLocalOfficeVersion(map<U_OFFICE_TYPE, string>& officeVersion)
{
	bool ret = true;
	mSoftwareInfo.clear();
	m_InstallInfo.clear();
...
bool CGetLocalMachineOfficeVersion::GetDefaultOpenMode(map<string, U_OFFICE_TYPE>& defaultOpenApp)
{
	bool ret = true;
		...
		for (LPCSTR ext : suffix)
		{
			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr))
				{
					...
					if (exePath.find("wps.exe") != -1 || exePath.find("et.exe") != -1 || exePath.find("wpp.exe") != -1)
					{

						if (hMS == 6 && lMS == 4)
						{
							if (exePath.find("wps.exe") != -1)
							{
								defaultOpenApp.insert(pair<string, U_OFFICE_TYPE>(ext, WPS_OFFICE_2009_WPS));
							}
							else if (exePath.find("et.exe") != -1)
							{
								defaultOpenApp.insert(pair<string, U_OFFICE_TYPE>(ext, WPS_OFFICE_2009_ET));
							}
							else if (exePath.find("wpp.exe") != -1)
							{
